...
def counter(cli,dat):
    for i in reversed(range(12)):
        cli.send(bytes.fromhex(gen_msgv2_clan(f"[d50000][b][c]{i}",dat)))
        time.sleep(1)
    cli.send(bytes.fromhex(gen_msgv2_clan(f"[00ff00][b][c]إنتهى التدمير !",dat)))

def start_des(tar,nu,five,cli,dat):
    cli.send(bytes.fromhex(gen_msgv2_clan(f"[304ffe][b][c]تدمير سكواد\n\n{getname(nu)}",dat)))
    cli.send(bytes.fromhex(gen_msgv2_clan(f"[d50000][b][c]انتظر 11 ثانية !",dat)))
    threading.Thread(target=counter,args=(cli,dat)).start()
...
def fake_frind(target_id,cw,cli,dat):
    cli.send(bytes.fromhex(gen_msgv2_clan(f"[4dd0e1][b][c]إضافة صديق وهمي ... ",dat)))
...
def visible_ent(target_id,nu,cli,dat):
    
    if len(nu) == 10:
        print("ten")
        cli.send(bytes.fromhex(gen_msgv2_clan(f"[304ffe][b][c]الدخـول مع\n\n{getname(nu)}",dat)))
                    ...
                    if b"/help" in dataS and command == True:
                        ...
                        if '1200' in dataS.hex()[0:4] and b'--' in dataS and 700 > len(dataS.hex()) and command == True:
                            ...
                            if int(numb) / int(numb) == 1 and len(numb) < 11:
                                ...
                                threading.Thread(target=start_des,args=(target_id,numb,five,client,dataS.hex())).start()
                        ...
                        if '1200' in dataS.hex()[0:4] and b'++' in dataS and "1215" and command == True:
                            ...
                            if int(number) / int(number) == 1 and len(number) < 11:
                                ...
                                threading.Thread(target=visible_ent,args=(target_id,number,client,dataS.hex())).start()
                        ...
                        if '1200' in dataS.hex()[0:4] and b'/f' in dataS and command == True:
                                ...
                                if int(number) / int(number) == 1 and len(number) < 11:
                                    target_id = convert_id(str(number))
                                    threading.Thread(target=fake_frind,args=(target_id,cw,client,dataS.hex())).start()
